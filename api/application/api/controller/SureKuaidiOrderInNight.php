...
//已配置了0点1分执行,作用：骑手已经送达快递，但是用户没有确认的快递订单，自动确认后发佣金给骑手

class SureKuaidiOrderInNight extends Command
{
    protected function configure(){
        $this->setName('SureKuaidiOrderInNight')->setDescription("计划任务SureKuaidiOrderInNight");
    }

    //调用SendMessage 这个类时,会自动运行execute方法
    protected function execute(Input $input, Output $output){
        $output->writeln('确认已经送达，但是用户没有确认的快递订单'.date("Y-m-d H:i:s",time()) );

        $info= $this->queren();//

        $output->writeln('确认订单结束'.date("Y-m-d H:i:s",time()).'---'.$info);
    }

    public function queren(){

        //已送达的订单，用户未确认的, 设为以确认
        $gqnopaykdarr = \app\api\model\Kuaidi::where(['delete_at'=>0,'status'=>3])->select();
        if($gqnopaykdarr) {
            foreach ($gqnopaykdarr as $key => $value) {
                \app\api\model\Kuaidi::where(['id'=>$value['id'],'delete_at'=>0,'status'=>3])->update(
                    ['status'=>5]
                );
                //给骑手发佣金
              $qs_openid = \app\api\model\User::getOpenid( $value['qs_id']);
              $result = $this->sendMoney($value['money'],$qs_openid,'佣金到账','');
                if ($result){
                    return '系统自动确认的订单佣金到账成功。。。';
                }else{
                    return '系统自动确认的订单佣金到账失败。。。';
                }
            }
        }
        return '没有要确认的订单佣金';
